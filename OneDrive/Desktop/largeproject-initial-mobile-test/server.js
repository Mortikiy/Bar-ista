...
const url =
  "mongodb+srv://alexslort:COP4331@cluster0.pmypipy.mongodb.net/LargeProject?retryWrites=true&w=majority";
...
app.post("/api/resetPassword", async (req, res, next) => {
  ...
  try {
    ...
    if (user.length > 0) {
      await db
        .collection("users")
        .updateOne({ _id: o_id }, { $set: { password: newPassword } });
...
app.post("/api/searchIngredient", async (req, res, next) => {
  ...
  const results = await db
    ...
    .find({ ingredient: { $regex: _search + ".*", $options: "i" } })
    .toArray();
...
app.post("/api/getDrinks", async (req, res, next) => {
  ...
  if (user.length > 0) {
    ...
    //console.log(bar);
    var ret = [];
    const makeDrink = await db
      .collection("Drinks")
      .find({ ingNeeded: { $not: { $elemMatch: { $nin: bar } } } })
      .toArray();
...
app.post("/api/addIngredientToBar", async (req, res, next) => {
  ...
  if (user.length > 0) {
    const result = await db
      .collection("ingredients")
      .find({ ingredient: ingName })
      .toArray();

    if (result.length > 0) {
      const add = await db
        .collection("users")
        .updateOne({ _id: user[0]._id }, { $addToSet: { bar: ingName } });
...
app.delete("/api/deleteIngredientInBar", async (req, res, next) => {
  ...
  if (user.length > 0) {
    ...
    if (result.length > 0) {
      const add = await db
        .collection("users")
        .updateOne({ _id: user[0]._id }, { $pull: { bar: ingName } });
      const updated = await db
        .collection("users")
        .find({ _id: o_id })
        .toArray();
...
app.post("/api/addFavorite", async (req, res, next) => {
  ...
  if (user.length > 0) {
    ...
    if (result.length > 0) {
      const add = await db
        .collection("users")
        .updateOne({ _id: user[0]._id }, { $addToSet: { savedDrinks: name } });
      const updated = await db
        .collection("users")
        .find({ _id: o_id })
        .toArray();
...
app.delete("/api/deleteFavorite", async (req, res, next) => {
  ...
  if (user.length > 0) {
    ...
    if (result.length > 0) {
      const add = await db
        .collection("users")
        .updateOne({ _id: user[0]._id }, { $pull: { savedDrinks: name } });
      const updated = await db
        .collection("users")
        .find({ _id: o_id })
        .toArray();
...
app.post("/api/getBar", async (req, res, next) => {
  ...
  if (user.length > 0) {
    ...
    var ret = [];
    const fullBar = await db
      .collection("ingredients")
      .find({ ingredient: { $in: bar } })
      .toArray();
